...
После сборки готовый файл находится здесь:

```text
dist/index.html
```

CSS и JavaScript встроены прямо в `index.html`, чтобы приложение открывалось с диска через `file://` без CORS-ошибок загрузки локальных ассетов.
...
2. При необходимости измените `Port`, `User` и `Password`.
3. Нажмите `OK`.
4. Следите за значением `Person Counts`.
5. При необходимости введите значение в `Reset Counts`.
6. Нажмите `reset`, чтобы вручную установить текущий счетчик.
...
Пароль хранится локально в браузере заказчика в `localStorage`, без отправки во внешние сервисы.
